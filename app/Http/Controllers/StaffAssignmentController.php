...
class StaffAssignmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $assignedCourses = $coursesQuery->orderBy('code')->get();

        $query = Assignment::with(['course', 'questions', 'submissions.student']);
        ...
        $allStaffAssignmentIds = Assignment::where('staff_id', $user->id)->pluck('id');
    ...
    public function bulkUpload(Request $request)
    {
        ...
        if ($request->filled('file_base64')) {
            ...
            if ($fileContent === false || empty($fileContent)) {
                return redirect()->back()->with('error', 'Failed to read uploaded file data. Please try again.');
            }
            ...
            if (!in_array($ext, ['xlsx', 'xls', 'csv', 'txt'])) {
                return redirect()->back()->with('error', 'Invalid file type (.'.$ext.'). Please upload an Excel (.xlsx, .xls) or CSV (.csv) file.');
            }
            ...
            if (!in_array($extension, ['xlsx', 'xls', 'csv', 'txt'])) {
                return redirect()->back()->with('error', 'Invalid file type (.'.$extension.'). Please upload an Excel (.xlsx, .xls) or CSV (.csv) file.');
            }
        ...
        try {
            $import = new AssignmentsImport(Auth::id());
            ...
            if ($import->importedCount > 0) {
                $message = "Successfully imported {$import->importedCount} MCQ question(s) across {$import->createdAssignmentsCount} assignment(s).";
                ...
                $errorMessage = 'No MCQ questions could be imported. Please make sure the Excel file has valid columns (course_code, assignment_title, question, option_a, option_b, option_c, option_d, correct_option).';
    ...
    public function show(Assignment $assignment)
    {
        ...
        $assignment->load(['course.department', 'course.enrollments', 'questions', 'submissions.student', 'submissions.answers.question']);

        $enrolledStudents = $assignment->course->enrollments;
        $submissionsByStudent = $assignment->submissions->keyBy('student_id');

        return view('staff.assignments.show', compact('assignment', 'enrolledStudents', 'submissionsByStudent'));
    }
